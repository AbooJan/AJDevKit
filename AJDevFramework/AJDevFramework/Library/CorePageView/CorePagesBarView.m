...
-(void)btnsPrepare{
    ...
    for (NSInteger i=0; i<_pageModels.count; i++) {
        ...
        //字体颜色
        [btn setTitleColor:DEFAULT_BTN_COLOR forState:UIControlStateNormal];
        ...
        if(_config.isBarBtnUseCustomWidth){
            btnW = _config.barBtnWidth + _config.barBtnExtraWidth;
        }
...
-(void)adjustLineViewFrameWithBtn:(CorePagesBarBtn *)btn isFirstBtn:(BOOL)isFirstBtn{
    ...
    CGFloat lineViewH = POSITION_LINE_HEIGHT;
...
-(UIView *)lineView{
    ...
    if(!_lineView){
        ...
        //设置颜色
        _lineView.backgroundColor = HIGHLIGHTED_COLOR;
